...
- `README.md:101`: Hardcoded personal domain remains. Environment-specific hostnames should be placeholders or overlays.
- `apps/grim-backend/overlays/production/ingress-openapi.yaml:12`: Hardcoded personal domain remains. Environment-specific hostnames should be placeholders or overlays.
- `apps/grim-backend/overlays/production/ingress-patch.yaml:15`: Hardcoded personal domain remains. Environment-specific hostnames should be placeholders or overlays.
- `apps/grim-backend/overlays/production/ingress-root-redirect.yaml:8`: Hardcoded personal domain remains. Environment-specific hostnames should be placeholders or overlays.
...
- `apps/prometheus/base/pv.yaml:12`: PersistentVolume uses hostPath. hostPath storage is acceptable for single-node lab use but not portable production storage.
...
- `cluster/argocd/prometheus-application.yaml:11`: Argo CD Application uses internal git:// source. The Git source of truth is inside the same cluster it manages; external Git remains deferred.
- `cluster/argocd/prometheus-application.yaml:18`: Argo CD Application enables automated prune/self-heal. Automated remediation is useful but increases the impact of a bad source commit.
